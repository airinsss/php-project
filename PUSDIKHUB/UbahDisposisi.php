 	 ...
 	 function query($query)
{
 	...
 	function Ganti($Data)
 {
 	...
 	$Data = query( "SELECT * FROM lembardisposisi WHERE No = '$no'")[0];
 	// var_dump($Brg);
 	...
 	if (isset($_POST["submit"])) {
 		...
 		if (Ganti ($_POST)> 0) {
 			...
 			document.location.href = 'TabelLembarDisposisi.php'
 			</script>
 			";
 		}
 		else {
...
<!--       <h1 class="logo me-auto"><a href="index.html">AiWeb<span>.</span></a></h1> -->
  ...
  <fieldset>
	...
	<center><form action="" method="post" id="payment">
		...
		<input type="text" name="no" id="no" value=" <?php echo$Data["No"] ?> ">
		</li>

		<li>
		<label for="NamaMateril">No Surat :</label>
		<input type="text" name="NoSurat" id="NoSurat" value=" <?php echo$Data["NoSurat"] ?> ">
		</li>

		<li>
		<label for="NoSurat">Tanggal Surat :</label>
		<input type="text" name="TglSurat" id="TglSurat" value=" <?php echo$Data["TglSurat"] ?> ">
		</li>

		<li>
		<label for="Tanggal">TanggalSuratMasuk:</label>
		<input type="date" name="TglSuratMasuk" id="TglSuratMasuk" value=" <?php echo$Data["TglSuratMasuk"]?> ">
		</li>

		<li>
		<label for="AsalSurat">Disposisi :</label>
		<input type="text" name="Disposisi" id="Disposisi" value=" <?php echo$Data["Disposisi"] ?> ">
	    </li>
